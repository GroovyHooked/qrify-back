...
router.post('/signup', async (req, res) => {
    try {
        ...
        if (!checkBody(req.body, ['lastname', 'firstname', 'company', 'email', 'password'])) {
        ...
        if (user) {
            ...
            if (savedUser) {
                ...
                res.json({ result: false, error: "Un problème est survenu lors de l'enregistrement en base de données" })
            }
        }
    } catch (error) {
        res.status(500).json({ error: "Un problème est survenu lors de l'inscription" });
    }
})
